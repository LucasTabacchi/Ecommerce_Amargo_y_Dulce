...
export default function CheckoutPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    body: JSON.stringify(createPayload),
      ...
      if (!createRes.ok) {
        console.error("Strapi create order error:", JSON.stringify(created, null, 2));
        throw new Error(pickErrorMessage(created, "No se pudo crear la orden"));
      }

      // Strapi v5: respuesta puede ser { data: {...} } o directamente {...}
      // Si viene doble-envuelta, desenvolver
      const orderData = created?.data ? created.data : created;
      const id: number | string | undefined = orderData?.id || orderData?.documentId;
      
      if (!id) {
        console.error("Strapi response:", JSON.stringify(created, null, 2));
        throw new Error("La orden se creó pero Strapi no devolvió un id o documentId.");
      }
      ...
      const normalizedItems = items
        .map((it) => ({
          title: it.title,
          qty: Number(it.qty ?? 1),
          price: Number(priceWithOff(it.price, it.off)),
        }))
        .filter((x) => x.qty > 0 && Number.isFinite(x.price) && x.price > 0);

      if (normalizedItems.length === 0) {
        throw new Error("No hay items válidos para MercadoPago (precio/cantidad).");
      }

      console.log("[checkout] Items antes de enviar a MP:", normalizedItems);
      ...
      console.log("[checkout] Respuesta de /api/mp/create-preference status:", prefRes.status);
